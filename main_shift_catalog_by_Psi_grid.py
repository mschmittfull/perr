    ...
    for specs_of_density_to_shift in opts['densities_to_shift']:
        ...
        if opts['plot_slices']:
            ...
            rescalefac = nbkit03_utils.linear_rescale_fac(
                opts['internal_scale_factor_for_weights'],
        ...
        for direction in range(3):
            ...
            Psi_rfields[
                direction] = nbkit03_utils.get_displacement_from_density_rfield(
                    ...
                    RSD=opts['RSD'],
                    RSD_line_of_sight=opts['RSD_line_of_sight'],
        ...
        out_delta, meshsource_attrs = paint_utils.weighted_paint_cat_to_delta(
            uni_cat,
            weight='Mass',
            weighted_paint_mode=opts['weighted_CIC_mode'],
            set_mean=0.0,
            Nmesh=opts['out_Ngrid'],
            verbose=opts['verbose'],
            to_mesh_kwargs={
                'window': 'cic',
                'compensated': False,
        ...
        if comm.rank == 0:
            print("%d: Linear rescalefac from a=%g to a=%g, rescalefac=%g" %
                  (comm.rank, opts['internal_scale_factor_for_weights'],
                   opts['out_scale_factor'], rescalefac))
        ...
        if not opts['RSD']:
            ...
            if opts['RSD_line_of_sight'] in [[0, 0, 1], [0, 1, 0], [1, 0, 0]]:
                RSDstring = '_RSD%d%d%d' % (opts['RSD_line_of_sight'][0],
        ...
        out_fname = os.path.join(
            ...
            (specs_of_density_to_shift['id_for_out_fname'],
             ...
             opts['out_scale_factor'], opts['Nptcles_per_dim'], opts['Nmesh'],
             opts['out_Ngrid'], opts['weighted_CIC_mode'], RSDstring))
